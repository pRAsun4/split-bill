...
function Field({
  ...
  keyboardType, maxLength, themeColors,
}: {
...
export default function EditProfileScreen() {
  ...
  // Initialise from real auth user
  ...
  const handleSave = async () => {
    ...
    // Update name via PATCH /users/me
    ...
    // Update profile fields via PATCH /users/me/profile
    const profileRes = await usersApi.updateMyProfile({
    ...
    if (profileRes.ok) {
      ...
      const meRes = await import("../../lib/api").then(async (m) => m.authApi.me(
        (await import("@react-native-async-storage/async-storage")).default.getItem(
          process.env.EXPO_PUBLIC_TOKEN_KEY ?? "splitty_auth_token"
        ).then((t) => t ?? "") as any
      ));
      // Simple approach: just update what we know changed
      if (user) {
        updateUser({
          ...user,
          name: name.trim(),
      ...
      Alert.alert("Error", profileRes.error);
    }
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: themeColors.bg }]}>
        ...
        <SafeAreaView edges={["top"]}>
          <View style={styles.headerRow}>
            ...
            <Animated.View style={{ transform: [{ scale: saveScale }] }}>
              <TouchableOpacity
                onPress={handleSave}
                style={[styles.saveBtn, (!hasChanges || saving) && { opacity: 0.45 }]}
          ...
          <View style={styles.avatarPreviewRow}>
            ...
            <View style={styles.avatarPreview}>
              ...
              <Text style={styles.avatarPreviewSub}>{user?.email ?? ""}</Text>
            </View>
          </View>
        </SafeAreaView>
      </LinearGradient>
      ...
      >
        ...
        >
          {/* Avatar color picker (local only — backend doesn't store this yet) */}
          ...
          <View style={styles.colorGrid}>
            {AVATAR_COLORS.map((c) => (
              <TouchableOpacity
                ...
                style={[styles.colorSwatch, { backgroundColor: c }, avatarColor === c && styles.colorSwatchActive]}
                activeOpacity={0.8}
              >
...
const styles = StyleSheet.create({
  ...
  avatarPreviewRow: { flexDirection: "row", alignItems: "center", gap: SPACE.md, paddingBottom: SPACE.sm },
  avatarPreview: {
  ...
  colorGrid: { flexDirection: "row", flexWrap: "wrap", gap: SPACE.sm },
  ...
  fieldLabel: { fontSize: FONT.xs, fontWeight: FONT.bold, letterSpacing: 0.5, marginBottom: SPACE.xs },
  fieldBox: { borderRadius: RADIUS.lg, borderWidth: 1.5, paddingHorizontal: SPACE.md, paddingVertical: SPACE.sm + 2 },
  fieldInput: { fontSize: FONT.base, fontWeight: FONT.medium, minHeight: 24 },
  charCount: { fontSize: FONT.xs, textAlign: "right", marginTop: 4 },
});